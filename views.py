...
class DeleteFromWatchlistView(APIView):
    permission_classes = [IsAuthenticated]

    def delete(self, request, movie_id):
        try:
            # Check if the movie exists in the user's watchlist
            watchlist_item = Watchlist.objects.get(user=request.user, tmdb_id=movie_id)
            watchlist_item.delete()  # Delete the item
            return Response({"message": "Movie removed from watchlist."}, status=status.HTTP_204_NO_CONTENT)
        except Watchlist.DoesNotExist:
            return Response({"error": "Movie not found in your watchlist."}, status=status.HTTP_400_BAD_REQUEST)
